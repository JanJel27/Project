import pygame,sys,gameover1,lvl_main,vic1,mysql.connector
...
def run1():
    ...
    pygame.mixer.music.load('music/theme_3.mp3')
    ...
    enemy_rect1 = pygame.Rect(860,5,22,30)
    enemy_rect2 = pygame.Rect(1200,100,22,30)
    enemy_rect3 = pygame.Rect(1960,218,22,30)
    ...
    while True: # game loop
        ...
        '''for background_object in background_objects:
            ...
            if background_object[0] == 0.5:
                pygame.draw.rect(display,(102,155,127),obj_rect)
            else:
                pygame.draw.rect(display,(102,155,127),obj_rect)'''
            ...
            if state==Running:
                ...
                if event.type == custom_event:
                    if player_rect.colliderect(chest_rect):
                        val1=(a,)
                        if who1:
                            mycursor.execute("UPDATE account SET money=money+?,level=3 WHERE id=1",val1)
                            con.commit()
                        elif who2:
                            mycursor.execute("UPDATE account SET money=money+?,level=3 WHERE id=2",val1)
                            con.commit()
                        elif who3:
                            mycursor.execute("UPDATE account SET money=money+?,level=3 WHERE id=3",val1)
                            con.commit()
                        pygame.mixer.music.fadeout(100)
                        pygame.mouse.set_visible(True)
                        vic1.victory2(star)
